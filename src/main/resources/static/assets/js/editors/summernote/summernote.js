(function (factory) {
  ...
  var async = (function () {
    ...
    var createImage = function (sUrl) {
      return $.Deferred(function (deferred) {
        $('<img style="height:300px">').one('load', function () {
        ...
        $imageDialog.one('shown.bs.modal', function (event) {
          ...
          $imageBtn.click(function (event) {
            ...
            //deferred.resolve($imageUrl.val());
             $.ajaxFileUpload({
	         url:"/sys/file/upload?module=editor", //上传文件的服务端
	         secureuri:false,  //是否启用安全提交
	         async:false,
	         dataType: 'json',   //数据类型  
	         fileElementId:"fileimage", //表示文件域ID
	         ...
	         if(data.status==200)
		     	 ...
		     	 { 
		     	 	deferred.resolve("/sys/file/getImage?attachId="+datalist[i].attachId);
		     	 }
                   ...
                   '<div class="modal-content">' +
                     ...
                     '<div class="modal-body">' +
                       '<div class="row-fluid">' +
                         ...
                         '<input class="note-image-input" type="file" id="fileimage" name="file" accept="image/*"/>' +
                       '</div>' +
                     '</div>' +
