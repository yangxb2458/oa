CKEDITOR.plugins.add('xseal', {
    init: function(editor){
        editor.addCommand('xseal', {
        	exec:function(){
        		$("#xplugin").html(xsealmodal);
        		...
        		$(".js-savetextueditbtn").unbind('click').click(function (){
        			...
        			var html = "<img xtype='xseal' " +
				        			"title='"+title+"' " +
									"name='"+name+"' " +
									"style='"+style+"' " +
									"defaultValue='"+defaultValue+"' " +
        							"src='/module/ckeditor/plugins/xseal/code.jpg'/>";
        			if(title=="")
					{
		...
		if (editor.contextMenu){
			editor.contextMenu.addListener(function(element,selection){
				...
				if ( isImg && element.getAttribute('xtype')=="xseal"){
					return {
...
var xsealmodal=['	 <div class="modal fade bs-example-modal-lg" id="xmodal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">',
				'        <div class="modal-dialog">',
				'            <div class="modal-content">',
				'                <div class="modal-header">',
				'                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>',
				'                    <h4 class="modal-title" id="myLargeModalLabel">签章组件</h4>',
				'                </div>',
				...
				'	                </div>',
				'	                <div class="form-group">',
				'	                    <label class="col-sm-2 control-label no-padding-right">校验字段</label>',
				'	                    <div class="col-sm-10">',
				'	                    <input type="text" class="form-control" id="defaultValue" name="defaultValue" placeholder="校验字段">',
				'	                	</div>',
				'	                </div>',
				'	                <div class="form-group">',
