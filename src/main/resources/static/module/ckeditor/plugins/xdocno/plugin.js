CKEDITOR.plugins.add('xdocno', {
    init: function(editor){
        editor.addCommand('xdocno', {
        	exec:function(){
    			$("#xplugin").html(xdocnomodal);
		...
		if (editor.contextMenu){
			editor.contextMenu.addListener(function(element,selection){
				...
				if ( isInput && element.getAttribute('xtype')=="xdocno"){
					return {
						xdocno : CKEDITOR.TRISTATE_OFF
					};
				}
				return null;
			});
...
var xdocnomodal=['	 <div class="modal fade bs-example-modal-lg" id="xmodal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">',
				'        <div class="modal-dialog">',
				'            <div class="modal-content">',
				'                <div class="modal-header">',
				'                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>',
				'                    <h4 class="modal-title" id="myLargeModalLabel">收发文文号</h4>',
				'                </div>',
				'                <div class="modal-body">',
				'                <form class="form-horizontal" role="form" id="xdocnoform">',
				...
				'	                </div>',
				'	                <div class="form-group">',
				'	                    <label class="col-sm-2 control-label no-padding-right">文号规则</label>',
				'	                    <div class="col-sm-10">',
...
function getDocumentNumberList()
{
	$.ajax({
		url : "/ret/documentget/getDocumentNumberListForSelect",
		type : "post",
		dataType : "json",
