function Designer(container) {
	...
	this.drawStart = function(x, y, info) {
		var node = $('<div node start noWrap class="node start_node"><img src="/module/flowdesigner/start_node.png" /><b>'
				+ info.prcsName + "</b></div>");
		node.attr("id", info.processId);
		node.data("data", info);
		node.css({
			left : x+"px",
			top : y+"px"
	...
	this.drawSimpleNode = function(x, y, info) {
		...
		var node = $('<div node simple noWrap class="node simple_node"><img src="/module/flowdesigner/simple_node.png"/>'+ info.prcsName+ "</div>");
		node.attr("id", info.processId);
		node.data("data", info);
		node.css({
			left : x+"px",
	...
	this.drawChildNode = function(x, y, info) {
		var node = $('<div  node child noWrap class="node child_node"><img style="width:20px" src="/module/flowdesigner/child_node.png" />'
				+ info.prcsName + "</div>");
		node.attr("id", info.processId);
		node.data("data", info);
		node.css({
