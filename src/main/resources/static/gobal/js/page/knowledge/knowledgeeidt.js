			...
			}else if(data.status=="100")
			{
			...
			}else
				{
					...
					for(var id in recordInfo)
					{
						if(id=="attach")
						{
							$("#knowledgeattach").attr("data_value", recordInfo.attach);
							createAttach("knowledgeattach",4);
						}else if(id=="sortId")
						{
							$("#sortId").attr("data-value", recordInfo.sortId);
							$("#sortId").val(getSortName(recordInfo.sortId));
						}else if(id=="content")
						{
							$("#content").code(recordInfo.content);
						}else
						{
...
{
	$.ajax({
		url : "/set/knowledgeset/updateKnowledge",
		type : "post",
		dataType : "json",
		data:{
			knowledgeId:knowledgeId,
