	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "200") {
				...
				for (name in v) {
					if (name == "sortLeave") {
						$("#sortLeave").attr("data-value",data.list.sortLeave);
						$.ajax({
							url : "/ret/projectbuildget/contract/getProjectBuildContractSortById",
							type : "post",
							dataType : "json",
...
function delPorjectBuildContractSort() {
	if (confirm("确定删除当前分类吗？")) {
		$.ajax({
			url : "/set/projectbuildset/contract/delProjectBuildContractSort",
			type : "post",
			dataType : "json",
			data : {
				sortId : $("#sortId").val()
...
function addsort() {
	...
	{
		$.ajax({
			url : "/set/projectbuildset/contract/insertProjectBuildContractSort",
			type : "post",
			dataType : "json",
			data : {
				sortNo : $("#sortNo").val(),
...
function updateErpBomSort() {
	...
	{
	$.ajax({
		url : "/set/projectbuildset/contract/updateProjectBuildContractSort",
		type : "post",
		dataType : "json",
		data : {
			sortId : $("#sortId").val(),
