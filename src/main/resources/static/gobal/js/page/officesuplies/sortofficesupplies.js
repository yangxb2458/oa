...
$(function() {
	...
	$("#cbut").unbind("click").click(function() {
		...
		$("#show_otherattach").html("");
		$("#otherattach").attr("data_value","");
		$("#otherattach").val("");
		$("#createbut").show();
		$("#updatabut").hide();
		$("#delbut").hide();
				...
				for (name in v) {
					if (name == "parentId") {
						$.ajax({
							...
							dataType : "json",
							data : {
								sortId : data.list.parentId
							},
							success : function(res) {
								if (res.status == "200") {
									if(res.list)
										{
										$("#sortLeave").val(res.list.sortId);
										$("#sortLeaveName").val(res.list.sortName);
										}else
										{
											$("#sortLeave").val("0");
											$("#sortLeaveName").val("");
					...
					{
						$("#otherattach").attr("data_value",data.list.logo);
						createAttach("otherattach","0");
					}else
					{
						$("#" + name).val(v[name]);
					}
...
function deleteOfficeSuppliesSort() {
	if (confirm("确定删除当前分类吗？")) {
		$.ajax({
			url : "/set/officesuppliesset/deleteOfficeSuppliesSort",
			type : "post",
			dataType : "json",
			data : {
				sortId : $("#sortId").val()
...
function addsort() {
	$.ajax({
		url : "/set/officesuppliesset/insertOfficeSuppliesSort",
		type : "post",
		dataType : "json",
		data : {
			sortNo : $("#sortNo").val(),
			sortName : $("#sortName").val(),
			parentId : $("#sortLeave").val(),
			remark : $("#remark").val(),
			logo:$("#otherattach").attr("data_value")
		},
		success : function(data) {
			if (data.status == 200) {
				top.layer.msg(data.msg);
...
function updateOfficeSuppliesSort() {
	$.ajax({
		url : "/set/officesuppliesset/updateOfficeSuppliesSort",
		type : "post",
		dataType : "json",
		data : {
			sortId : $("#sortId").val(),
