...
function query() {
	$("#myTable").bootstrapTable({
		url : '/ret/hrget/getHrTrainRecordApprovedList',
		method : 'post',
...
function createOptBtn(recordId,status) {
	var html ="";
	html+="<a href=\"javascript:void(0);apply('" + recordId + "')\" class=\"btn btn-primary btn-xs\">审批</a>&nbsp;&nbsp;";
...
function apply(recordId)
{
	...
	$(".js-save").unbind("click").click(function(){
		$.ajax({
			url : "/set/hrset/approvedHrTrainRecord",
			type : "post",
			dataType : "json",
			data:{
				recordId:recordId,
