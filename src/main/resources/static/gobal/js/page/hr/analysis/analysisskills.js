...
function createeCharts()
{
	...
	if(dataType=="1")
	{
		$("#echartTitle").html("技能等级");
	}
	if(echartType=="1")
	{
		getSkillsPieForAnalysis(dataType,deptId);
	}else if(echartType=="2")
	{
		getSkillsBarForAnalysis(dataType,deptId);
	}else if(echartType=="3")
	{
		getSkillsTableForAnalysis(dataType,deptId)
	}
		
}

function getSkillsTableForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getSkillsTableForAnalysis",
		type : "post",
		dataType : "json",
...
function getSkillsPieForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getSkillsPieForAnalysis",
		type : "post",
		dataType : "json",
		data:{dataType:dataType,deptId:deptId},
		success : function(data) {
...
function getSkillsBarForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getSkillsBarForAnalysis",
		type : "post",
		dataType : "json",
		data:{dataType:dataType,deptId:deptId},
		success : function(data) {
