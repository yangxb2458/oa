...
function createeCharts()
{
	...
	if(dataType=="1")
	{
		$("#echartTitle").html("奖惩类型");
	}else if(dataType=="2")
	{
		$("#echartTitle").html("处理事项");
	}
	if(echartType=="1")
	{
		getIncentivePieForAnalysis(dataType,deptId);
	}else if(echartType=="2")
	{
		getIncentiveBarForAnalysis(dataType,deptId);
	}else if(echartType=="3")
	{
		getIncentiveTableForAnalysis(dataType,deptId)
	}
		
}

function getIncentiveTableForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getIncentiveTableForAnalysis",
		type : "post",
		dataType : "json",
...
function getIncentivePieForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getIncentivePieForAnalysis",
		type : "post",
		dataType : "json",
		data:{dataType:dataType,deptId:deptId},
		success : function(data) {
...
function getIncentiveBarForAnalysis(dataType,deptId)
{
	$.ajax({
		url : "/ret/echartshrget/getIncentiveBarForAnalysis",
		type : "post",
		dataType : "json",
		data:{dataType:dataType,deptId:deptId},
		success : function(data) {
