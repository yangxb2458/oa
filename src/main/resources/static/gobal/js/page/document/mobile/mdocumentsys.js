...
function selectdocument(Obj)
{
	...
	$(".xdocumentSelectokbtn").unbind("click").click(function(){
		...
		for(var i=0;i<arr.length;i++)
			{
			$("span[name='"+parentName+"']").append("<div class='alert alert-info fade in js-documentlist-"+parentName+"' style='margin-bottom: 2px;margin-top:2px;'>" +
					"<button class='close' data-dismiss='alert'>×</button>" +
					"<a onclick='readdocument(this)' data-value='"+arr[i].runId+"' style='cursor: pointer;color: #FFFFFF;'>"+arr[i].flowTitle+"</a></span>"
			);
			}
