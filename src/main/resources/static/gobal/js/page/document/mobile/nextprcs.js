...
$(function(){
    ...
    $(documentProcess.autoSendType=="1")
    {
    	$("#sendToUserDiv").show();
    }
...
function createOpUserAndPrcs(prcsList) {
	...
	}else if(documentProcess.concurrentFlag=="2")
	{
		$("input:checkbox[prcs_type!=2]").each(function(){
...
function getDocumentUserList(processId) {
		$.ajax({
			url : '/ret/documentget/getUserInfoInPrivForMobile',
			type : "post",
			dataType : "json",
			data:{
				processId:processId,
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "200") {
				...
				top.layer.msg(data.msg);
				open("/mobile/document/mydocumentlist","_self");
			} else if (data.status == "100") {
				top.layer.msg(data.msg);
			} else {
				console.log(data.msg);
