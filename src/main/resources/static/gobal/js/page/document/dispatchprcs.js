$(function() {
	...
	$(".js-doAllDocumentUrge").unbind("click").click(function(){
		doAllDocumentUrge();
	})
...
{
	$.ajax({
		url : "/ret/documentget/getCorssDocumentFlowList",
		type : "post",
		dataType : "json",
...
function query1()
{
	 $("#myTable1").bootstrapTable({
	      url: '/ret/documentget/getProcessDocumentFlowList',
	      method: 'post',
...
function queryParams1(params) {
    var temp = {
        ...
        sort: params.sort,
        sortOrder:params.order,
        pDocumentRunNo:$("#pDocumentRunNo").val(),
...
{
	var html="<a href=\"javascript:void(0);doDocumentUrge('"+runId+"')\" class=\"btn btn-sky btn-xs\" >催办</a>";
...
function doAllDocumentUrge()
{
	var selected = $('#myTable1').bootstrapTable('getSelections');
	var ids = new Array();
	for(var i=0;i<selected.length;i++){
	ids[i]=selected[i].id;
	}
	doDocumentUrge(ids.join(","));
}

function doDocumentUrge(runId)
{
	$.ajax({
		url : "/set/documentset/doDocumentUrge",
		type : "post",
		dataType : "json",
		data:{runIds:runId},
		success : function(data) {
...
{
	$.ajax({
		url : "/ret/documentget/getCanTaskBackRunProcessList",
		type : "post",
		dataType : "json",
		data:{runId:runId},
		success : function(data) {
...
$.ajax({
	url : "/set/documentset/doTaskBack",
	type : "post",
	dataType : "json",
...
function isCantTaskBack(runId)
{
	var returnValue;
	$.ajax({
		url : "/ret/documentget/isCantTaskBackFlag",
		type : "post",
		dataType : "json",
		async : false,
		data:{runId:runId},
