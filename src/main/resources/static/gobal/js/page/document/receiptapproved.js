...
function query()
	{
		 $("#myTable").bootstrapTable({
		      url: '/ret/documentget/getGoProcessList1',
		      method: 'post',
...
{
	$.ajax({
		url : "/set/documentset/delDocument",
		type : "post",
		dataType : "json",
...
function entrust(runId,runProcessId)
{
	...
	$.ajax({
		url : "/ret/documentget/getEntrustUser",
		type : "post",
		dataType : "json",
		data:{runId:runId,runProcessId:runProcessId},
		success : function(data) {
...
{
	open("/app/core/document/dowork?runId="+runId+"&runProcessId="+runProcessId,"_self");
}
