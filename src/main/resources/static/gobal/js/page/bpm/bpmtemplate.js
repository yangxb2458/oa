...
function query3() {
	$("#myTable3").bootstrapTable({
		url : '/ret/bpmget/getBpmTemplateList',
		method : 'post',
...
function del(templateId)
{
	if(window.confirm('你确定要删除模版吗？')){
		$.ajax({
			url : "/set/bpmset/deleteBpmTemplate",
			type : "post",
			dataType : "json",
			data : {
...
function addTemplate() {
	$.ajax({
		url : "/set/bpmset/insertBpmTemplate",
		type : "post",
		dataType : "json",
		data : {
			sortNo : $("#sortNo").val(),
...
function updateTemplate(templateId) {
	$.ajax({
		url : "/set/bpmset/updateBpmTemplate",
		type : "post",
		dataType : "json",
		data : {
			templateId:templateId,
...
function edit(templateId)
{
	...
	$("#templateattach").attr("data_value","");
	$.ajax({
		url : "/ret/bpmget/getBpmTemplateById",
		type : "post",
		dataType : "json",
		data : {
			templateId : templateId
