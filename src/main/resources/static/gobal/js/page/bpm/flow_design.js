...
function otherChildNode(node) {
	...
	$(".right-box").html(html);
	$.ajax({
		url : '/ret/bpmget/getAlternativePrcsList',
		dataType : "json",
		type : "POST",
		data : {
			flowId : flowId,
		...
		success : function(data) {
			if(data.status=="200")
			{
				...
				$(".js-toOtherPrcsSetsave").unbind("click").click(function(){
					...
					$.ajax({
						url : "/set/bpmset/updateBpmProcess",
						dataType : "json",
						type : "POST",
						data : {
							flowId : flowId,
...
function addChildNode(node) {
	bootbox.prompt("请输入新建子流程步骤名称", function (result) {
        if (result ===null) {
        	...
        	var id = node.attr("id");
        	$.ajax({
        		url : "/set/bpmset/insertBpmProcess",
        		type : "POST",
        		dataType : "json",
        		data : {
        			flowId : flowId,
...
function setChildNodeinfo(node) {
	...
	$.ajax({
		url : "/ret/bpmget/getBpmChildProcessPrcs",
		dataType : "json",
		type : "post",
		data : {
			processId : processId
...
function remove(node) {
	if (window.confirm("确定删除该节点吗？")) {
		var processId = node.attr("id");
		$.ajax({
			url : "/set/bpmset/deleteBpmProcess",
			dataType : "json",
			type : "POST",
			data : {
				flowId : flowId,
...
function saveLayout() {
	...
	$.ajax({
		url : "/set/bpmset/saveBpmProcessLayout",
		dataType : "json",
		type : "post",
...
{
	$.ajax({
		url : "/ret/bpmget/getBpmPluginsListForProcess",
		dataType : "json",
		type : "POST",
		async : false,
		success : function(data) {
...
function setChildFlow(processId)
{
	...
	$.ajax({
		url : "/set/bpmset/setBpmChildProcess",
		dataType : "json",
		type : "POST",
		data:{
			processId:processId,
...
function getRemindNextUser()
{
	...
	json.wxSms=$("input[name='remindNextUser_wxSms']:checked").val();
...
function getRemindCreateUser()
{
	...
	json.wxSms=$("input[name='remindCreateUser_wxSms']:checked").val();
	return  JSON.stringify(json);
}
function getRemindParticipant()
{
	...
	json.wxSms=$("input[name='remindParticipant_wxSms']:checked").val();
	return  JSON.stringify(json);
}