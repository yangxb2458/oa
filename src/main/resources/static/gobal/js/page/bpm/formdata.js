	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "200") {
				...
				$("#prcsName").html("当前步骤：" + data.list.bpmProcess.prcsName);
				$(".js-saveandnextbtn").attr("data-value-process-id", data.list.bpmProcess.processId);
				// $(".js-saveandnextbtn").attr("data-value-next-prcs", data.list.bpmProcess.nextPrcs);
...
function getNextPrcs() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (bpmProcess.concurrentFlag == "0"||bpmProcess.concurrentFlag == "1") {
...
function createOpUserAndPrcs(prcsList) {
	...
	for (var i = 0; i < prcsList.length; i++) {
		...
		var ishide="";
		if(bpmProcess.concurrentFlag!="2")
		{
			ishide = "display:none;";
		}
		if (prcsList[i].prcsType != "2") {
			html += '<div id="prcs_select_' + prcsList[i].processId + '" style="'+ishide+'background-color: #f5f5f5;margin-bottom:10px;" class="row">'
					...
					+ '<a href="javascript:void(0);" style="float:right;margin-top: 10px;" class="btn btn-purple js-bpmPrcsSelectOpuser" auto-accountid='+prcsList[i].sPrcsAutoAccountId
	...
	$(".js-bpmPrcsSelectOpuser").each(function() {
		$(this).unbind("click").click(function() {
			bpmOpUserSelect(this);
		});
	});
	...
	$(".js-changeTitle").unbind("click").click(function(){
		$.ajax({
			...
			success : function(data) {
				if (data.status == "200") {
					top.layer.msg(data.msg);
					$("#flowTitle").html($("#bpmFlowTitle").val());
